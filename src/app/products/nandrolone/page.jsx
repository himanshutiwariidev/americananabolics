...
export const metadata = {
  title: "Nandrolone Products | AmericanAbolics",
  description: compoundCategories.nandrolone.description,
};

export default function NandroloneProductsPage() {
  const nandroloneProducts = getProductsByCategory("nandrolone");
    ...
    <main className="min-h-screen bg-white py-16 md:py-24">
      <div className="mx-auto max-w-5xl px-6">
        <nav className="mb-8 text-sm text-gray-400">
          ...
          <span className="text-gray-600">Nandrolone Products</span>
        </nav>

        <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          {compoundCategories.nandrolone.title}
        </h1>
        <p className="mt-6 max-w-2xl text-base leading-relaxed text-gray-500 sm:text-lg">
          {compoundCategories.nandrolone.description}
        </p>

        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {nandroloneProducts.map((product) => (
            <Link
              key={product.slug}
